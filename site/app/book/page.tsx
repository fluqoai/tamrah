...
export default function BookingPage() {
  ...
  useEffect(() => {
    ...
    setParams(next);
    if (!next.checkIn || !next.checkOut) { setLoading(false); return; }
    supabase.rpc('check_unit_availability', { requested_unit_id: UNIT_ID, requested_check_in: next.checkIn, requested_check_out: next.checkOut, requested_guests: Number(next.guests) })
      .then(({ data }) => { setAvailable(Boolean(data?.[0]?.available)); setLoading(false); });
  }, []);
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_390px]">
          <section className="rounded-[1.6rem] border border-border bg-[#fffaf2] p-5 shadow-sm sm:p-8">
            ...
            <form className="grid gap-5 sm:grid-cols-2">
              ...
              <Button disabled className="h-13 w-full rounded-xl text-base sm:col-span-2">الدفع وتأكيد الحجز — {nights * 400} ر.س</Button>
            </form>
          </section>

          <aside className="h-fit rounded-[1.6rem] border border-border bg-[#fffaf2] p-5 shadow-sm lg:sticky lg:top-6">
            <div className="relative h-52 overflow-hidden rounded-2xl"><Image src="/images/master-window.jpeg" alt="شقة تمرة" fill className="object-cover" /></div>
            <h2 className="mt-5 text-xl font-semibold">شقة تمرة العائلية</h2><p className="mt-2 flex items-center gap-2 text-xs text-muted-foreground"><CheckCircle2 className="size-4 text-[#394034]" /> حجز مؤكد بعد الدفع</p>
            ...
            <div className="mt-5 space-y-3 text-sm"><div className="flex justify-between"><span>{nights} ليالٍ × 400 ر.س</span><span>{nights * 400} ر.س</span></div><div className="flex justify-between"><span>الضريبة</span><span>0 ر.س</span></div><div className="flex justify-between border-t border-border pt-4 text-lg font-semibold"><span>الإجمالي</span><span>{nights * 400} ر.س</span></div></div>
          </aside>
        </div>
      </div>
    </main>
